...
#创建目录
vl -s 50 httprobe.txt | grep -v "\[50" | grep -v "\[404" | grep -oP "http.*" >> /root/script/3_httprobe/httprobe1.txt ; sort -u httprobe1.txt -o httprobe1.txt
...
grep -oP "http.*" $output/3_endpoint_JS.txt > $output/3_endpoint_JS123.txt ; vl -s 50 $output/3_endpoint_JS123.txt | grep -v "\[50" | grep -v "\[404" | grep -oP "http.*" > $output/3_endpoint_JS.txt ; rm $output/3_endpoint_JS123.txt ; sort -u $output/3_endpoint_JS.txt -o $output/3_endpoint_JS.txt
vl=`ps -A | grep vl | awk '{print $1}' | sed 's/[[:space:]]//g'`
for line in $vl
do
kill -9 $line
